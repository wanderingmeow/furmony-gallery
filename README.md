...
- **瀑布流浏览**：虚拟化行主序网格；直接以**文档（html/body）为滚动容器**（iOS 状态栏点按 / PC Home 键均可回到顶部）；右侧原生滚动条；滚动驱动的首卡片预览 chip（时间序→`#ID`，价格序→`¥金额`）；右下角「回到顶部 / 刷新」按钮（平滑动画）
...
- **详情 bottom-sheet**：iOS 式底部弹层，滑入 / 滑出，**触摸拖拽可关闭**（阈值或快速下滑判定，未达则回弹）；画廊两层交叉淡入 + 加载 spinner；全屏缩放（pinch-zoom 已禁用，`+/−` 缩放，Esc / X 关闭）；信息表；「前往官网」按钮（无论锁定状态均显示）
...
npm run test    # vitest（image / listingCache / filter / scrollRestore，32 tests）
```
